...
class Classify :
    def __init__(self):
        landmark_path = "./DeepFakeDetector/landmarks/"
        assert os.path.exists(landmark_path), "Landmark path does not exist. Please extract the landmarks firstly."
        test_samples, test_samples_diff, _, _, test_sv, test_vc = self.get_data_for_test(landmark_path, 1, block_size)
        ...
        model.load_weights('./DeepFakeDetector/model_weights/deeper/g1.h5')
        model_diff.load_weights('./DeepFakeDetector/model_weights/deeper/g2.h5')
        ...
        prediction_video = self.merge_video_prediction(mix_predict, test_sv, test_vc)
        ...
        for i, pd in enumerate(prediction_video):
            if pd >= 0.5:
                ...
                self.label = "Real"
            print("{}-Prediction label: {}; Scores:{}".format(video_names[i], self.label, pd))
        print("#------------End------------#")
    

    def get_data_for_test(self, path, fake, block):  # fake:manipulated=1 original=0
    ...
    def merge_video_prediction(self, mix_prediction, s2v, vc):
        prediction_video = []
        pre_count = {}
