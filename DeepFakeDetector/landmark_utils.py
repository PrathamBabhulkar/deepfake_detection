...
predictor = dlib.shape_predictor("./DeepFakeDetector/shape_predictor_68_face_landmarks.dat")
...
class LandmarkUtils:
    def shape_to_face(self, shape, width, height, scale=1.2):
        """
    ...
    def check_and_merge(self, location, forward, feedback, P_predict, status_fw=None, status_fb=None):
        num_pts = 68
        check = [True] * num_pts
    ...
    def detect_frames_track(self, frames, fps, use_visualization, visualize_path, video):
        ...
        for i in tqdm(range(frames_num)):
            frame = frames[i]
            face, shape = self.predict_single_frame(frame)
            if face == 0:
            ...
            face, face_size = self.shape_to_face(shape, frame_width, frame_height, 1.2)
        ...
        for i in tqdm(range(locations_sum - 1)):
            ...
            merge_pt, check, P_predict = self.check_and_merge(locations_seg, forward_pts, feedback_pts, P_predict, status_fw,
                                                        status_fb)
